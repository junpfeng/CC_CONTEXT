...
- **Agent 1（文档阅读）**：阅读需求文档 `docs/version/{version_id}/{feature_name}/feature.json`（若不存在则读 `feature.md`）+ 项目宪法 `P1GoServer/.claude/constitution.md` 和 `freelifeclient/.claude/constitution.md`。同时检查 `docs/version/{version_id}/{feature_name}/idea.md` 是否存在 `### 锁定决策` 子节——若存在，将其逐条提取作为**不可变约束**返回，后续设计必须原样采纳这些决策
- **Agent 2（现有设计调研）**：阅读 `docs/knowledge/` 下与本功能相关的现有设计文档
...
> **锁定决策保护**：若 Agent 1 返回了 `### 锁定决策` 条目，这些决策已由用户在 new-feature 阶段确认，**不纳入提问范围**。提问仅针对锁定决策未覆盖的维度和 `### 待细化` 中的项目。
...
当功能涉及客户端（freelifeclient）改动时，**必须**在 plan 中追加 Unity MCP 验收测试用例（至少 1 个 [TC-XXX]）。缺少 TC 会导致 auto-work Stage 4-B MCP 验收被跳过，客户端表现问题无法被自动发现。

**用例格式**（写入 `plan/verify.json` 或 plan.json 的 `acceptance_tests` 字段）：
  ...
  "acceptance_tests": [
    {
      "id": "TC-001",
      "name": "用例名称",
      "preconditions": ["已登录", "在某场景"],
      "steps": [
        {"action": "MCP 操作描述", "verify": "预期结果 + 验证方式（screenshot-game-view / gameobject-find / script-execute / console-get-logs）"}
      ],
      "exception_scenarios": ["网络断开时的表现", "重复操作时的表现"]
    }
  ]
}
```
