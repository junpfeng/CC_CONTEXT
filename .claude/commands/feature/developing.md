...
4. **无法自动匹配时**：使用 AskUserQuestion 向用户确认，列出已有的版本目录和功能目录供选择（自动化模式下 hook 会硬拦截 AskUserQuestion，此时应从 `docs/version/` 下最新版本目录搜索含 `plan.json` 的子目录自动推导）

**engine_name 规则：**
- 如果解析出 engine_name 且不为空：在编码完成后，将本功能的特性描述合并到 `docs/knowledge/{engine_name}/` 下的工程文档中（详见第七步）
...
- **Agent 4（工程文档，仅 engine_name 非空时）**：阅读 `docs/knowledge/{engine_name}/` 下的文档
...
**自动推断模式**（`{FEATURE_DIR}/idea.md` 存在 `## 确认方案` 时）：
- 从 plan.json 的 `file_list` 中每个条目的 `side` 字段（`server` / `client` / `both`）自动推断实现端，无需询问用户
- 如果 `file_list` 无 `side` 字段，从文件路径推断（`P1GoServer/` → server，`freelifeclient/` → client）
- 文件清单长度不论多少，均一次性全部实现（子 feature 场景下无人交互）

**交互确认模式**（无 `## 确认方案` 且为用户直接调用时）：
...
**自动降级模式**（无 `## 确认方案` 但 AskUserQuestion 被 hook 拦截时）：
- 实现端：默认两端都做（plan 包含两端设计时），plan 仅涉及单端时只做该端
- 文件清单：一次性全部实现，不分批
...
- **发现可优化项时主动确认**：编码过程中如果发现已有代码存在可以优化或改进的地方（如命名不规范、逻辑可简化、性能可提升、模式不一致等），不要自行修改，而是用 AskUserQuestion 向用户描述发现的问题和建议的改进方案，由用户决定是否在本次一并修改。**例外**：若 `{FEATURE_DIR}/idea.md` 存在 `## 确认方案`（子 feature 场景），则不提优化建议，只做 plan 要求的

**DDRP 依赖发现与上报**（编码过程中遵守 `.claude/rules/ddrp-protocol.md`）：

编码某个 task 时发现依赖的系统/模块/接口不存在，按规模分级处理：

| 规模 | 判断标准 | 处理 |
|------|----------|------|
| 内联 | ≤50 行、单文件、无外部依赖 | 当前 task 直接实现，develop-log 记录 `[DDRP-INLINE]` |
| 子任务 | 50-300 行、1-3 文件、自包含 | 暂停 → 实现 → 编译 → 恢复，记录 `[DDRP-SUBTASK]` |
| 子系统 | >300 行 或 3+ 文件 或 有自身依赖 | 写 ddrp-req 文件后继续尝试（编译失败后引擎正常 discard） |

**子系统上报格式**：写入 `{FEATURE_DIR}/ddrp-req-{TASK_ID}.md`（worktree 环境下用主工作区绝对路径）：
...
**关键规则**：
- 子系统必须上报，禁止 developing 内部 spawn 进程
- 写入 ddrp-req 后继续尝试当前 task，不要停下来等
- `核心能力` 必须写具体类型名和方法签名，不能只写概括性描述
...
当用户提供了 `engine_name` 参数时，将本功能的**核心特性描述**合并到工程文档 `docs/knowledge/{engine_name}/` 中。

**目标文件**：`docs/knowledge/{engine_name}/` 下的 `.md` 文件（通常每个目录只有一个主文档）。
