...
5. **无法自动匹配时**：使用 AskUserQuestion 向用户确认（自动化模式下 hook 会硬拦截 AskUserQuestion，此时应从 `docs/version/` 下最新版本目录搜索含 `develop-log.md` 的子目录自动推导）
...
- **Agent 1（plan + feature 上下文）**：阅读 `docs/version/{version_id}/{feature_name}/plan.json`（或旧版 `plan.md`）及 `plan/` 子目录中的所有子文件（.json 或 .md），以及 `feature.json`（或旧版 `feature.md`）。同时读取 `docs/knowledge/consolidation-index.md`（若存在），提取 `## Error Patterns` 和 `## Review Checklist Additions` 作为补充审查清单。返回：功能需求摘要、预期文件清单、关键设计决策、补充审查清单
...
5. **需求对齐**（如有 feature.json）：逐条检查 requirements[].acceptance_criteria 是否在代码中有对应实现，未覆盖的标记为 CRITICAL
...
| 检查项 | 严重级 |
|--------|--------|
| 事务边界是否清晰？哪些操作必须同事务？ | CRITICAL |
| 失败回滚逻辑是否完整（中间步骤失败时前序操作能否撤销）？ | CRITICAL |
| 幂等性：同一请求重复发送是否安全？有无幂等键？ | HIGH |
| 并发控制：共享资源加锁是否正确？锁范围是否最小？有无死锁风险？ | HIGH |
| 超时处理：等待外部调用的超时是否设置？超时后状态是否清理？ | HIGH |
